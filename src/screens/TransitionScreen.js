...
export default function TransitionScreen (props){
    ...
    const [prelievo, setPrelievo] = useState(false)
    const [categoria, setCategoria] = useState("Entrate");
    ...
    const showCalendar = () => {
        return(
            <View style = {{width: '90%'}}>
                <Calendar
                  onDayPress = {(day) => {onDayPress(day); setDate(day); setTimeout(function(){setModalCalendarVisible(false)}, 300)}}
                  hideExtraDays
                  markedDates = {{
                    [calendar.selected]: {
                      selected: true,
    ...
    const checkDati = () => {
        ...
        if(categoria === "Entrate" && prelievo){
            Alert.alert('Dati errati', 'Inserire la categoria');
            return false;
        }
        if(date === ""){
            Alert.alert('Dati errati', 'Inserire la data');
            return false;
        }
        return true;
    };
    ...
    const timeFunct = () => {
        ...
        let timeStamp = firebase.firestore.Timestamp.fromDate(newDate);
        if(myToday.valueOf() ===  newDate.valueOf()){
            timeStamp = firebase.firestore.Timestamp.now();
        }
        return timeStamp;
    }

    //carica sul database le informazioni relative al prelievo e ricarica
    const addTextTransazione = async () =>{
        let data = {}
        if(prelievo){
            data = {
                authorID: userID,
                    ...
                    <View style = {{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingRight: 10}}>
                        ...
                        <View style = {{flexDirection: 'row-reverse', alignItems: 'center'}}>
                            ...
                            {date === "" ? (
                                ...
                                <> 
                                    <Text style = {{fontSize: 14, fontWeight: 'bold'}}>{convertDate(date.dateString)}</Text> 
                                </>
                            )}
                            </Text>
                        </View>
